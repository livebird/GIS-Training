...
Col9=x Double
Col10=y Double</pre></div><p class="small"><strong>Instructor verification item:</strong> Microsoft’s reference lists <code>CharacterSet</code> values ANSI and OEM; whether ArcGIS Pro reads a UTF-8 CSV correctly with this line, and whether a byte-order mark helps or hurts, must be checked in the installed version. If Gujarati shows wrong, remove the <code>CharacterSet</code> line, retry, and record both outcomes.</p></div>
  ...
  <div class="lab-card"><h4>Part 4 — “After” snapshot and comparison</h4><div class="checklist">
    <label><input type="checkbox" data-save="p4a"><span><strong>11.</strong> Add each output to the map. For each, fill an “after” column beside your “before”: field names and types, count, geometry type, CRS as reported, extent, nulls per field, the two Unicode samples, the identifier sample, the date/time samples, the three known records.</span></label>
  ...
  <div class="table-wrap"><table>
    ...
    <tbody>
      <tr><td>Gujarati shows as <code>?</code> or boxes in the Pro table</td><td>Non-UTF-8 code page on read; the <code>CharacterSet</code> line; presence/absence of a BOM</td><td>Try without the <code>CharacterSet</code> line; try saving with a UTF-8 BOM; record which combination worked</td></tr>
...
function pageInit() {
  ...
  const R = [
    { id: "R-A", who: "R-A — contractor", say: "“Our old desktop tool only reads shapefiles.”", need: "Points, IDs and types; they will not use timestamps; they read English only.", note: "Shapefile is justified <em>as a delivered copy</em>. Predict: 7 names shortened (document the mapping), null score → 0 (declare it or agree a sentinel), time dropped, Gujarati at risk unless a Unicode code page is written and honoured. Send all companion files + readme." },
    { id: "R-B", who: "R-B — web developer", say: "“Give me GeoJSON for the prototype.”", need: "Draw six points on a schematic grid in a browser, no basemap; needs the Gujarati labels.", note: "GeoJSON <em>syntax</em> only under a written <strong>prior arrangement</strong> (RFC 7946 §4): the coordinates are grid metres, not degrees, so the file is not standard GeoJSON and must not be handed on. Nulls, Unicode, long names and the ISO timestamp string all survive." },
    { id: "R-C", who: "R-C — internal GIS team", say: "“One file for the field tablet with everything intact.”", need: "Full fidelity: nulls, timestamps with their instant, both scripts, long names; single-user edits.", note: "GeoPackage (or a file/mobile geodatabase). Predict: everything kept; the timestamp becomes UTC <code>Z</code> if typed as date/time (an adaptation to document — SL-0055’s date changes); the CRS registers as an undefined Cartesian grid (<code>srs_id −1</code>) or your assigned local reference — record which." }
  ];
  ...
  function exp() {
    ...
    const checks = [
      ...
      { p: "asset_type_gu for SL-0113", v: () => ["સ્ટ્રીટલાઇટ", "સ્ટ્રીટલાઇટ", "intact only with a Unicode .cpg honoured by the reader; else garbled"] },
