<?php $page = ['title' => '7.4 Richer containers: GeoPackage and GeoTIFF', 'chapter' => 7, 'module' => '7.4']; require __DIR__ . '/../partials/head.php'; ?>
  ...
  <div class="table-wrap"><table>
    ...
    <tbody>
      ...
      <tr><td>Req 5, Table 1</td><td>Column types are fixed. <code>TEXT</code> is UTF-8/UTF-16; <code>DATE</code> is <code>YYYY-MM-DD</code>; <code>DATETIME</code> is “YYYY-MM-DDTHH:MM:SS.SSSZ with … Z suffix for coordinated universal time (UTC)”.</td><td>Gujarati safe; long names safe; real nulls. But a timestamp with an offset <strong>must become UTC</strong> — <code>2026-08-30T02:10:00+05:30</code> → <code>2026-08-29T20:40:00.000Z</code>. An adaptation you must write down.</td></tr>
...
function pageInit() {
  ...
  const T = {
    ...
    assets_ch7: { sql: "SELECT fid, asset_id, legacy_code, asset_type_gu, condition_score, last_inspection_at FROM assets_ch7;", cols: ["fid", "asset_id", "legacy_code", "asset_type_gu", "condition_score", "last_inspection_at"], rows: F7.rows.map((r, i) => [i + 1, r.asset_id, r.legacy_code, r.asset_type_gu, r.condition_score, r.last_inspection_at ? toUTC(r.last_inspection_at).replace("Z", ".000Z") : null]), note: "Your data. Long names kept; Gujarati kept; NULL kept (TR-0301); legacy_code kept as TEXT with its zeros; last_inspection_at shown as GeoPackage DATETIME in UTC — SL-0055 now reads 2026-08-29. (Simulated: if you exported the timestamp as text it would stay as typed.)" }
  };
